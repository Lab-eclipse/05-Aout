...
public class CopyOfGestionFiliere extends JFrame {
...
//	declaration des variable de la connexion dans la classe motPasseIndicateur
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CopyOfGestionFiliere frame = new CopyOfGestionFiliere();
...
//					La methode setString permet de recuperer les donnees dans db
...
//				permet d afficher dans textField un element selectionne dans la table
				...
				String id = table.getModel().getValueAt(ligne, 1).toString();
				
				String sql = "SELECT * FROM filieres where id_filiere = '"+id+"'";
				...
				try
				{
					...
					if(resultat.next())
					{
						nomField.setText(resultat.getString("nomfliere"));
...
//		Code sql permettant de selectionner des elements dans la table utilisateurs de la db 
		String sql = "select id_user, username, password from utilisateurs";
		try {
			prepare = (PreparedStatement) cnx.prepareStatement(sql);
			resultat = prepare.executeQuery();
